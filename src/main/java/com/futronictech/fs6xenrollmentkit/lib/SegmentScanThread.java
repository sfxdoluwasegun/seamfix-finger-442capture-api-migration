 ...
 * @author slyeung
 */
public class SegmentScanThread  implements Runnable {
    ...
    public SegmentScanThread(FPDevice devFP)
    {
        m_devFP = devFP;
    }
    
    public boolean StartSegment(Object syncRoot, JLabel lableShowArea)
    {        
        m_SyncRoot = syncRoot;
        m_LabelShowArea = lableShowArea;
        if( !InitDevice() )
            return false;
        m_WorkedThread = new Thread( this, "SegmentScanThread" );
        //m_devFP.m_WorkerThread = m_WorkerThread;
    ...
    public boolean InitDevice()
    {
        ...
        width[0] = height[0] = 0;
        if( !m_devFP.PrepareSegmentation(width, height))
            return false;
    ...
    private void SegmentScanProcess()
    {
        m_devFP.DoScan();
    }
}